...
We can see that the number of cylinders appears to have a high impact on whether the mpg is above the median. Let's investigate the number of cars for each cylinder.
...
We can see now that the amount of cars are not equally distributed across the different cylinders. Therefore thinking back to our previous graph, let us disregard cylinders 3 and 5 due to their extremely low occurrences.

```{r}
Auto_mpg |> 
  filter(cylinders == c(4, 6, 8)) |> 
  ggplot(aes(x = cylinders, fill = mpg01)) + 
  geom_bar(position = "fill") +  
...
If we were to predict which methodology will do best for classifying here, I'd say it will likely be LDA and QDA. Logistic regression will suffer from the separation in each class being high. Naive Bayes makes the assumption that features within a class are independent of each other, that will clearly be violated and performance will greatly suffer as a result. K-nearest-neighbors will not do great due to N not being high relative to the number of predictors.
